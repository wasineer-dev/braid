#
# Generate the observation count as Spoke model
#
...
def CountSpokeModel(nProteins, listBaits, listIndices):

    mObserved = np.zeros(shape=(nProteins, nProteins), dtype=int)
    ...
    mTrials = np.zeros(shape=(nProteins, nProteins), dtype=int)
    nDim = nProteins*nProteins
    a = np.arange(nDim).reshape(nProteins,nProteins)
    
    print('Trials: Array Dimension ' + str(mTrials.shape))
    for bait in listBaits:
        dim = bait
        mTrials[bait, 0:dim:1] += 1
        if dim < nProteins:
            rows = np.arange(nProteins-dim) + dim
            cols = [bait]*len(rows)
            mTrials[rows, cols] += 1
    
    mPreComputed = interactionProbability(0.3, 0.4, 0.01)
    mPosterior = np.zeros(shape=(nProteins, nProteins), dtype=float)

    for i in np.arange(nProteins):
        for j in np.arange(i+1):
            t = mTrials[i][j]
            s = mObserved[i][j]
            mPosterior[i][j] = mPreComputed[t][s]
